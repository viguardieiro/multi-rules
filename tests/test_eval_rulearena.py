...
class TestPromptConstructionAirline:
    ...
    def test_basic_prompt_no_example(self, sample_problem, sample_rules):
        prompt, boostable = build_prompt_airline(sample_problem, sample_rules, use_example=False)
        assert "Rule 1:" in prompt
        assert "Alice is flying" in prompt
        assert "<example>" not in prompt

    def test_prompt_with_example(self, sample_problem, sample_rules):
        prompt, _ = build_prompt_airline(sample_problem, sample_rules, use_example=True)
    ...
    def test_boostable_text_is_reference_rules(self, sample_problem, sample_rules):
        _, boostable = build_prompt_airline(sample_problem, sample_rules, use_example=False)
...
class TestPromptConstructionNBA:
    ...
    def test_basic_prompt_no_example(self, sample_query):
        ...
        prompt, boostable = build_prompt_nba(sample_query, rules, use_example=False)
    ...
    def test_all_placeholders_replaced(self, sample_query):
        ...
        prompt, _ = build_prompt_nba(sample_query, rules, use_example=True)
        assert "$reference_rules" not in prompt
...
class TestPromptConstructionTax:
    def test_basic_prompt_fills_forms(self):
        ...
        problem = problems[0]
        prompt, boostable = build_prompt_tax(problem, use_example=False)
...
class TestBuildBoostConfig:
    ...
    def test_none_strategy_returns_none(self, gpt2_tokenizer):
        result = build_boost_config("none", "some text", gpt2_tokenizer, "rules", 2.0)
        assert result is None

    def test_zero_bias_returns_none(self, gpt2_tokenizer):
        result = build_boost_config("uniform_rules", "some text", gpt2_tokenizer, "rules", 0.0)
        assert result is None
    ...
    def test_uniform_rules_creates_config(self, gpt2_tokenizer):
        ...
        config = build_boost_config("uniform_rules", text, gpt2_tokenizer, rules, 2.0)
    ...
    def test_unknown_strategy_raises(self, gpt2_tokenizer):
        with pytest.raises(ValueError, match="Unknown boost strategy"):
            build_boost_config("fancy_boost", "text", gpt2_tokenizer, "rules", 1.0)
...
class TestSamplesJson:
    def test_save_and_load(self, tmp_path):
        samples = [
            ...
            {
                "sample_idx": 1,
                "sample_input": "Bob flies to JFK",
                "predicted_answer": None,
                "ground_truth_answer": 800,
                "is_correct": False,
                "model_output": "I don't know.",
                "generation_finished": False,
                "output_length_chars": 13,
                "input_length_tokens": 480,
                "output_length_tokens": 2000,
                "sample_time_seconds": 5.5,
                "boost_metadata": "No boosting (baseline)",
            },
        ]
